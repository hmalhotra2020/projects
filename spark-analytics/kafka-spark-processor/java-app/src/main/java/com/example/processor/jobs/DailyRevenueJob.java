...
/**
 * Spark Streaming job that aggregates purchase quantity and amount
 * by date, hour, and fuel type per day.
 * Reads from Kafka topic 'purchases', writes to Postgres table 'daily_revenue'.
 * Mirrors the original PPEvaluatorJob (MySQL -> Postgres, Spark 2.4 -> 3.5).
 */
public class DailyRevenueJob implements Serializable {
    ...
    public DailyRevenueJob(String kafkaBootstrap, String postgresUrl, String postgresUser, String postgresPass) {
    ...
    public void evalStream() {
        ...
        rowStream.foreachRDD(rdd -> {
            if (!rdd.isEmpty()) {
                ...
                // purchaseTime is expected as "yyyy-MM-dd HH:mm:ss"
                Dataset<Row> withTs = df.withColumn("purchaseTs",
                        to_timestamp(df.col("purchaseTime")));
                Dataset<Row> aggregated = withTs
                        .groupBy(
